...
// 任务卡：T04。禁止全局两两比较：比较只发生在 (时间桶 × 地理簇) 内部。
    ...
    static func groups(
        ...
        for (bucketIndex, bucket) in timeBuckets.enumerated() {
            ...
            for unit in geoUnits {
                for component in hammingComponents(unit, hashByID: hashByID) where component.count >= 2 {
                    groups.append(
                        CandidateGroup(
                            id: "cand-\(bucketIndex)-\(component[0].localIdentifier)",
                            members: component,
                            reason: "时间×地理×pHash"
                        )
                    )
